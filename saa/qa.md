...
（A 社のベータ・リスクは TOPIX 先物の売りによって打ち消され、市場並みのベータでアクティブ・リターン（アルファ）を狙うことができる。残る A 社のアクティブ・リスクの水準はベータ・リスクと無関係に決定、管理すればよい。
...
年齢との関係：若年期はリスク資産の投資比率を高くし、中高年期には債券などにシフトさせることにより、リスク資産の投資比率を引き下げる。
...
IR は超過リターンをトラッキング・エラーで割ったものなので、  
IR = {-10.5-(-12.7)}/2.28 = 0.96

<<解説>>

IR = アクティブ・リターン（α）/トラッキング・エラー（TE）

ここでのアクティブ・リターンの定義ははっきりしていて、「**ポートフォリオのベンチマークに対する平均超過リターン**」である。

なお、図表 1 の「年率化 α」は（注2）に「CAPM を用いて評価した α を年率換算したもの」とあるから、これはいわゆる「CAPM アルファ」つまりジェンセンのアルファ。これは**インフォメーション・レシオで用いるアクティブ・リターンを表す α とは全く別物**。
...
各月の超過リターンを計算すると、それぞれ 0.00 %, +0.50 %, +1.60 % で、その平均値は +0.70 % である。したがって、その標準偏差は、

√({(0.00 - 0.70)^2 + (0.50 - 0.70)^2 + (1.60 - 0.70)^2}/3) = 0.6683

より、トラッキング・エラーは 0.67 % である。
</details>
...
このヘッジファンド H の最大ドローダウンを % 表示で小数第 1 位まで求めよ。

<details>
<summary>答えを見る</summary>

最大ドローダウンとは、**そのファンドのピークから見た場合の最大下落率**を指す。ここでは、NAV = 100 億円のファンドが最大 163 億円になって、139 億円まで下落したので、最大ドローダウンは (139 - 163) / 163 = -0.147 より - 14.7 % である。
...
- **生き残りバイアス、残存者バイアス**：パフォーマンスの悪いファンドは解約されてしまうため、ヘッジファンド・インデックスのパフォーマンスが過大評価されるバイアス。
- 分類バイアス：マネージャーの運用手法や対象が様々であるにもかかわらず、サンプル数確保の観点からユニバース分類が限定された数にならざるをえないバイアス
- **遡及バイアス**：新規参入ファンドのリターンを遡及的に算入することによるバイアス。
- **自己選択バイアス**：リターンの開示をしないファンドによるバイアス。

（詳しい解説）
* 生き残りバイアス：消滅したファンド等を計算から除外することによる上方バイアス。低収益のため報告を行わないファンドやすでに消滅したファンド、成功報酬の獲得が困難となったためにマーケティングを停止したファンドなどのデータが勘案されず、結果的にヘッジファンド・ユニバースのリターンが過大評価される傾向があるとされる。

* 遡及バイアス：新規組入れファンドのリターンを遡及的に算入することによる上方バイアス。マネジャーが（複数のファンドから）過去の収益率が高いファンドのデータのみを選択に提供する、あるいは都合のよいデータ期間を選ぶことによるバイアスがこれにあたる。

* 自己選択バイアス：リターン実績を開示しないファンドが存在することによる上方・下方バイアス。成功したファンドが（新規顧客獲得の必要がないために）データ提供を停止することによる下方バイアスなどがこれにあたる。
...
- 過去のパフォーマンスが将来のパフォーマンスを保証するものではないということ。
- 過去のボラティリティやシャープ・レシオだけでは、真のリスクがわからないということ。
- パフォーマンスが良いときは、実は収益機会が減少しているときとなるので（「投資タイミングのパラドックス」）、過去のリターンが良いときは最悪のタイミングかもしれないという点。
- 運営規模の小さいヘッジファンドの場合、オペレーショナル・リスクがある点。
- パフォーマンスに関する提示データが適正であること。
- 運用担当者の離職リスクが小さいこと。

> Note: **投資タイミングのパラドックス**  
> 過去のパフォーマンス実績がよかったということは、すでに全体として収益機会が減衰している過程である可能性があり、こういう局面で資金を投じても過去の良好なパフォーマンスは得られず、むしろ損失となる場合が多いということ。
</details>
...
ESG に積極的に取り組む企業を中心に投資する（購入する）投資主体が増加することは、これら ESG 銘柄の、他の投資家にとっての換金性（流動性）を高める。その結果、潜在的な資金提供者の層も広がり、当該企業の市場での資金調達に有利に働く。

> 当該企業の株式が積極的に購入され株価が上昇すれば、資金調達コストが低下し、NPV > 0 の投資機会も増すため、企業業績の改善が図られる。これはさらなる株価上昇へとつながる。
...
運用手法の観点にかかわらず、投資ユニバースやウェイトに制約をかけることにより、投資機会集合が狭まるため、効率的フロンティアが右下へと押し広げられ、パフォーマンスが低下する可能性がある。  
与えられた環境の中で最善の結果（＝経済的・金銭的効果であるリターン）を出すべく努力をすることが求められるのに、基金は自ら意図的に投資機会集合を狭め、パフォーマンス悪化を招きかねない行為をとる、あるいは委託先運用期間にとらせることになり、加入者に対する忠実義務に反する可能性がある。

<<解説>>

「現代ポートフォリオ理論の観点から」ということで、①投資ユニバースがどうなるのか、②銘柄間のリターンの相関がどうなるのか、③これが分散効果にどのように作用するか、といった道筋をたどれば解答しやすい。

別の答案文：ESG 積極型の類似した企業で投資ユニバースが構成され、**各銘柄間のリターンの相関係数は高くなる**可能性がある。非 ESG 型企業が排除されることで**分散効果は弱まり**、**効率的フロンティアは右下に後退**する。これは非 ESG 企業を投資ユニバースに含めた場合よりも、リスク調整後リターンが劣化することを意味する。

基金は運用受任者として、リスク・リターン最適化を図ることが第一義的職務である。ESG への取組といった情報による投資ユニバース決定は、業績や財務活動といった客観的事実とは無関係に投資対象を制限する。これは加入者の利益を侵害する恐れがあり、忠実義務に反すると考えられる。
...
r_p - r_TOPIX = α + (β - 1)(r_TOPIX - r_f) + ε

Var(r_p - r_TOPIX) = Var(α + (β - 1)(r_TOPIX - r_f) + ε)  
= (β - 1)^2 * Var(r_TOPIX - r_f) + Var(ε) + 2 * (β - 1) * Cov(r_TOPIX - r_f, ε)  
= (0.96 - 1)^2 (4.15%)^2 + (0.36%)^2 + 0  
= 0.157156 * 10 ^(-4)

トラッキング・エラー（年率換算）= √(Var(r_p - r_TOPIX) * 12) = .. = 0.01372 = 13.7 %
</details>
...
ファーマ・フレンチのファクター・エクスポージャーは、所定の手続きで作成したポートフォリオのリターンを用いて作成したファクターに分析対象のリターンを回帰させた回帰係数である（時系列回帰）。一方、ファンダメンタル・ファクター・モデルでは時価総額や PBR などの属性値の銘柄間格差をエクスポージャーとして用いている。
</details>
...
上海に上場している企業のファンダメンタルズと、その他の市場で取引されている株式の企業のファンダメンタルズは、直接の相関関係は小さく、実際の起こった株価変動の強い連動性は説明できない。
</details>
...
上海上場株の保有者も取引者も中国の国内個人投資家で、世界市場における影響力のある投資家は異なった主体であるため、上海の暴落により中国の投資家心理や実際の財務状況が傷んだとしても、他の市場で他の投資家が、精神的あるいは財務的に追い込まれて投げ売りをする必要はないから。
</details>
...
問3. 上海市場ではバブルが起こりやすいという認識が正しいとすると、そのバブルはノイズトレーダーリスクの考え方を使ってどのように説明できるか。

<details>
<summary>答えを見る</summary>

上海市場には個人投資家を中心にノイズトレーダーが相対的に多く存在し、彼らの影響力が多いために、株価が効率的な水準から乖離することが多くなる。一方、同市場では海外投資化を中心に多様な考え方を持つ投資家の参入に制限があることから、裁定取引が十分には起こりにくいため、ノイズトレーダーの影響力が支配する市場となって、裁定取引の恐れもないことから、個人投資家はある程度安心してバブルに乗ることができるから。
</details>
...
上海市場以外では市場参加者にノイズトレーダーが少なく、裁定取引が比較的十分に行われる。ファンダメンタルズから乖離した価格が形成・維持される可能性が低く、バブルは起こりにくいと考えられる。
</details>
...
ノイズトレーダーは上海株式市場の暴落を見て、ファンダメンタルズに基づく合理的な根拠もなく株式を売り、その他の株式市場も下落した。ポジティブフィードバック・トレーダーはこれを「売りのサイン」と捉え呼応して株式を売却し、さらにポジティブフィードバック・トレードが続くことを見越した合理的投資家も株式を売却した。こうしたサイクルが結果的に暴落につながったため、連動しているように見えた。
</details>
...
金融取引に関する諸規制を出来る限り少なくし、資産価格がすぐに効率的な価格に収斂する予想を形成することにより、ノイズトレーダーリスクを最小化する。この結果、ポジティブフィードバック・トレードも起こりにくくなる、合理的投資家の便乗トレードも減るはずである。
</details>
...
（1）効率的市場仮説とは、**現実の証券市場は「すべての証券の市場価格が常にその投資価値に等しい」**という意味で効率的であるとする仮説のことを指す。

ファーマは、情報効率性の水準をウィーク型、セミストロング型、ストロング型の 3 類型に分類した。そして、

- ウィーク型：過去の証券価格
- セミストロング型：すべての公開情報
- ストロング型：インサイダー情報を含むすべての情報

を証券価格は反映して的確に形成されている状況と定義した。そのため、それぞれの水準で小売てうきな市場では、該当する情報を活用して証券投資しても、過大なリターンを平均的に得ることはできないという示唆が導かれる。

（2）セイラーの上記の計測結果は、3〜5 年程度の計測期間における負け組に投資すれば、その後中長期的に見て相対的に高いリターンを上げ得ることを示唆しており、①過去の証券価格動向から将来のリターンに関する示唆が得られるため、②ウィーク型の効率的市場仮説に対する反証と位置づけることができる。
...
<img src="https://public.muragon.com/m5f4h0tt/bc35sns1.png">

- 総資産額ではなく、参照点からのプラス幅（得）とマイナス幅（損）で評価される。
- プラスの領域ではリスク回避的、マイナスの領域ではリスク愛好的になっている。
- マイナス（損）のほうがプラス（得）の領域よりも傾きがきつく、得の領域での満足度よりも損失の領域での苦痛のほうが絶対値ベースでは大きい。

</details>

評価関数 v(x) において、セイラーは、x1, x2 という複数の金銭的な損得が発生した場合、v(x1 + x2) を「統合」、v(x1) + v(x2) を「分離」と称し、意思決定者が都合よく、統合と分離を使い分けるという評価方法を提示した。この評価関数に従う投資家にとって、100 万円の得と 10 万円の損が発生した場合には、統合 v(100-10) と分離 v(100) + v(-10) のうち、どちらの価値が高くなると考えられるか、価値が高いほうを理由とともに答えよ。

<details>
<summary>答えを見る</summary>

価値が高くなるのは、統合 v(100 - 10)

プロスペクト理論の評価関数では、得の領域よりも損の領域のほうが関数の傾きが大きくなっており、**できるだけ損を認識しないほうがよい**という示唆が導かれる。そのため、10 万円の損を認識すると、100 万円の得から得た価値のうち、かなりの部分が目減りしてしまう。そのため、このような評価関数に従う意思決定者にとっては、分離を用いて 100 万円の得と 10 万円の損が発生したと認識して得られる価値 v(100) + v(-10) よりは、統合を用いて 90 万円の得だけが発生したと認識して得られる価値 v(90) のほうが高くなると考えられる。

</details>

## 信用リスクモデル

問1. 信用リスクモデルの中には、「構造モデル」と「誘導型モデル」の 2 つの類型があるが、それぞれのモデルの考え方と特徴を簡単に説明せよ。

<details>
<summary>答えを見る</summary>

- 構造モデル（Structural Model）：企業の保有する資産価値が負債価値を下回り、債務超過に陥った状態をもってデフォルトと捉える方法論。社債発行企業の B/S に基づいて、企業価値を原資産、負債額面を権利行使価格としてオプション理論を援用する。
- 誘導型モデル（Reduced Form Model）：デフォルトの確率分布およびデフォルト確率、その他信用リスクを測定するパラメータを外生的に与えて信用リスクのある資産の価値を計算し、これに基づいて信用リスクを推計する。
</details>

問2. 信用リスクモデルの立場から考えると、株式はオプションと見なすことができる。その場合、①原資産、②行使価格に対応するもの、③オプションのタイプ（コール、プット）を示せ。また、株式をオプションと見なすことができる理由を説明せよ。

<details>
<summary>答えを見る</summary>

①原資産：企業価値、②行使価格：負債額面、③オプションのタイプ：コール

理由：株式の CF は企業価値が負債額面を超えていれば正であり、理論上は無限大となる。一方、負債額面以下であればゼロであり、**株主有限責任**の原則により出資額以上の責任は負わないため。
</details>

問3. 信用リスクのない残存期間 1 年の割引国際の現在の価格が 98 円、B 格付けの同じ残存期間の割引社債の価格が 94 円である。

（1）2 つの資産の間の関係を示す確実性等価係数（アルファ係数）はいくらか。

（2）この社債のリスク中立的なデフォルト確率は 10 % と見積もられている。このとき、リスク中立的な予想回収率はいくらか。
<details>
<summary>答えを見る</summary>

（1）**確実性等価係数**とは、不確実性を伴う資産の価値がそうでない資産価値の何割に当たるかを示すもの。したがって、それは信用リスクのある割引債価格と信用リスクのない割引国債価格との比で示される。

94 / 98 = 0.95918 ≒ 0.9592

（2）予想回収率を x % とする。

((1-0.1) * 100% + 0.1 * x %) / (100/98) = 94
